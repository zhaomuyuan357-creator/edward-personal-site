...
现在还有一个房间式互动版本：<https://zhaomuyuan357-creator.github.io/edward-personal-site/v2/>。它把网站分成「关于我、AI 学习室、项目展厅、摄影与影像、成长记录、联系我」六个房间；根目录的简洁版仍然保留，方便手机或不支持 3D 的设备访问。

房间版操作：先进入并打开入口大门，在走廊用 W/A/S/D 或方向键移动；走近门后按 E 或点「打开」，镜头会随开门进入小房间。房间内拖动鼠标可转视角，再按「查看」读内容、「返回走廊」继续探索。手机上可上下滑动前后移动、左右滑动转视角，也可以直接点门。
...
5. 确认效果后，双击 `更新网站.command`。它会检查内容、提交并推送到 GitHub，Pages 会自动发布新版本。
